  ...
  return (
    <div className="bg-hero-img h-[50vh] relative">
      <div className="flex h-full justify-around items-center relative">
        <button className="bg-white  rounded-full w-10 h-10 flex items-center justify-center absolute -left-3">
          <img src={left} alt="left" srcset="" />
        </button>
        {data.map((data) => (
          <HeroCards key={data.title} title={data.title} />
        ))}
        <button className=" bg-white rounded-full w-10 h-10 flex items-center justify-center absolute -right-3">
